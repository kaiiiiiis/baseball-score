...
function handleRequest(e) {
  ...
  try {
    ...
    // --- action=standings：試合結果シートから順位表を集計して返す（★新規） ---
    if (e.parameter && e.parameter.action === 'standings') {
      const ss = SpreadsheetApp.getActiveSpreadsheet();
      const sheet = ss.getSheetByName('試合結果');
      const table = {};   // チーム名 -> {team,games,wins,losses,draws}
      const games = [];   // 試合結果の一覧（新しい順）

      if (sheet && sheet.getLastRow() >= 2) {
        const rows = sheet.getRange(2, 1, sheet.getLastRow() - 1, 6).getValues();
        rows.forEach(r => {
          const date = r[0], teamA = r[1], scoreA = r[2], teamB = r[3], scoreB = r[4], winner = r[5];
          if (!teamA || !teamB) return;

          [teamA, teamB].forEach(name => {
            if (!table[name]) table[name] = { team: name, games: 0, wins: 0, losses: 0, draws: 0 };
          });
          table[teamA].games++;
          table[teamB].games++;
          ...
          games.push({
            ...
            teamA, scoreA, teamB, scoreB, winner
          });
        });
      }

      const standings = Object.keys(table).map(name => {
        const t = table[name];
        const decided = t.wins + t.losses;
        const pct = decided > 0 ? (t.wins / decided) : 0;
        return {
          team: t.team, games: t.games, wins: t.wins, losses: t.losses,
          draws: t.draws, pct: pct.toFixed(3)
        };
      }).sort((a, b) => (b.pct - a.pct) || (b.wins - a.wins) || a.team.localeCompare(b.team, 'ja'));

      games.sort((a, b) => new Date(b.date) - new Date(a.date));

      output.setContent(JSON.stringify({ status: 'ok', standings, games: games.slice(0, 50) }));
      return output;
    }
    ...
    // --- シート「試合結果」（★新規：順位表のもとになる試合ごとの結果） ---
    let gameSaved = false;
    if (data.game && data.game.teamA && data.game.teamB) {
      let gSheet = ss.getSheetByName('試合結果');
      if (!gSheet) {
        gSheet = ss.insertSheet('試合結果');
        gSheet.appendRow(['日時', 'チームA', 'Aスコア', 'チームB', 'Bスコア', '勝者']);
        gSheet.getRange(1,1,1,6).setFontWeight('bold').setBackground('#7c2d12').setFontColor('#ffffff');
      }
      const g = data.game;
      gSheet.appendRow([
        g.date ? new Date(g.date) : new Date(),
        g.teamA, g.scoreA || 0,
        g.teamB, g.scoreB || 0,
        g.winner || '引き分け'
      ]);
    ...
    output.setContent(JSON.stringify({ status: 'ok', updated: players.length, pitchers: pitchers.length, gameSaved }));
    return output;
